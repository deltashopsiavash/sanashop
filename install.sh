...
if [[ -z "$DOMAIN" || -z "$ACME_EMAIL" || -z "$TELEGRAM_BOT_TOKEN" || -z "$TELEGRAM_ADMIN_IDS" || -z "$DJANGO_SUPERUSER_PASSWORD" ]]; then
  echo "دامنه، ایمیل، توکن ربات، آیدی مدیر و رمز پنل الزامی هستند."
  exit 1
fi
