...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "home":
        ...
        await q.edit_message_text({"edit_price": "قیمت جدید به تومان:", "edit_stock": "موجودی جدید:", "edit_photo": "عکس جدید محصول را بفرستید:"}[action])
...
async def message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if flow == "new_category" and text:
        ...
        value = int(text.replace(",", "")); field = "price" if flow == "edit_price" else "stock"
...
async def photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not allowed(update) or context.user_data.get("flow") not in ("new_product", "edit_photo", "setting_logo"): return
