...
from shop.models import Category, ContentPage, DiscountCode, HeroSlide, Order, PaymentReceipt, Product, SiteSetting
...
def main_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⚙️ تنظیمات فروشگاه", callback_data="settings"), InlineKeyboardButton("🔐 بکاپ", callback_data="backups")],
        [InlineKeyboardButton("📄 صفحات سایت", callback_data="pages"), InlineKeyboardButton("🎞 بنرها", callback_data="banners")],
        [InlineKeyboardButton("🎟 کد تخفیف", callback_data="discounts")],
    ])
...
async def show_product(q, product):
    ...
    kb = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📦 موجودی", callback_data=f"edit_stock:{product.id}")],
...
async def show_order(q, order):
    ...
    kb = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⬅️ سفارش‌ها", callback_data="orders")],
    ])
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "home":
        ...
        cat = await Category.objects.aget(pk=int(data.split(":")[1])); count = await cat.products.acount()
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("✏️ تغییر نام", callback_data=f"rename_category:{cat.id}"), InlineKeyboardButton("⏸ توقف/فعال", callback_data=f"toggle_category:{cat.id}")],
            [InlineKeyboardButton("🗑 حذف", callback_data=f"delete_category:{cat.id}"), InlineKeyboardButton("⬅️ دسته‌ها", callback_data="categories")],
        ])
        ...
        kb = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("📢 نوار اعلان", callback_data="setting_announcement")], back_button(),
        ])
        await q.edit_message_text(f"⚙️ تنظیمات\nنام: <b>{store.site_name}</b>\nپرداخت: {store.get_payment_mode_display()}\nارسال: {store.shipping_fee:,} تومان", parse_mode="HTML", reply_markup=kb)
    elif data in ("setting_name", "setting_card", "setting_announcement", "setting_shipping", "setting_free_shipping", "setting_logo"):
        context.user_data.clear(); context.user_data["flow"] = data
        prompts = {"setting_name": "نام جدید سایت را بفرستید:", "setting_card": "شماره کارت و نام صاحب حساب را با | جدا کنید:\nمثال: 6037... | سیاوش قادری", "setting_announcement": "متن جدید نوار اعلان را بفرستید:", "setting_shipping": "هزینه ارسال جدید به تومان (فقط عدد):", "setting_free_shipping": "حداقل خرید برای ارسال رایگان به تومان:", "setting_logo": "لوگوی جدید را به‌صورت عکس بفرستید:"}
...
async def photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not allowed(update) or context.user_data.get("flow") not in ("new_product", "edit_photo", "setting_logo", "banner_add"): return
