...
if [[ ${EUID} -ne 0 ]]; then echo "این دستور باید با sudo/root اجرا شود."; exit 1; fi
read_value(){ local key="$1" prompt="$2" default="${3:-}" secret="${4:-0}" value="${!1:-}"; if [[ -z "$value" ]]; then if [[ "$secret" == "1" ]]; then read -r -s -p "$prompt" value; echo; else read -r -p "$prompt" value; fi; fi; printf -v "$key" '%s' "${value:-$default}"; }
...
if [[ -z "$DOMAIN" || -z "$ACME_EMAIL" || -z "$DJANGO_SUPERUSER_PASSWORD" || -z "$SMTP_HOST" || -z "$SMTP_USER" || -z "$SMTP_PASSWORD" ]]; then echo "فیلدهای الزامی کامل نیستند."; exit 1; fi
export DEBIAN_FRONTEND=noninteractive
apt-get update
apt-get install -y ca-certificates curl git openssl
if ! command -v docker >/dev/null 2>&1; then curl -fsSL https://get.docker.com | sh; fi
systemctl enable --now docker
if [[ -e "$APP_DIR" ]]; then echo "$APP_DIR از قبل وجود دارد."; exit 1; fi
